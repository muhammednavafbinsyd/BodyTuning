...
function Data() {
  ...
  useEffect(() => {
    const getListservices = async () => {
      ...
      try {
        const response = await axios.get(`${BaseUrl}/adminroute/serviceGet`);
  ...
  const remove = async (id) => {
    try {
      await axios.delete(`${BaseUrl}/adminroute/serviceDelete/${id}`);
      closeDeleteDialog();
      getListservices();
    } catch (error) {
      console.log(error);
  ...
  const Description = ({ description }) => (
    <SoftBox display="flex" flexDirection="column">
      <SoftTypography variant="caption" fontWeight="medium" color="text" style={{width:"31rem"}}  >
        {description}
      </SoftTypography>
    </SoftBox>
  );
